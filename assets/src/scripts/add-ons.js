if (wps_js.isset(wps_js.global, 'request_params', 'page') && wps_js.global.request_params.page === "plugins") {
    jQuery(document).ready(function () {
        ...
        const license_buttons = document.querySelectorAll('.js-wpsms-addon-license-button');
        const addon_items = document.querySelectorAll('.js-wpsms-addon-check-box');
        const retry_addon_install_btn = document.querySelectorAll('.js-addon-retry-btn');
        const select_all = document.querySelector('.js-wpsms-addon-select-all');
        ...
        const toggleActiveAll = () => {
            if (active_addon_plugin_btn && active_addon_plugin_btn.length > 0) {
                if (active_all) {
                    active_all.classList.remove('wpsms-hide')
                }
            }
        }
        toggleActiveAll();
        ...
        const processSlugsSequentially = (index) => {
            select_all.classList.add('wpsms-hide');

            addon_items.forEach(function (item, i) {
                if (item.checked && i > index) {
                    const containStatus = item.parentElement.parentElement.querySelector('.wpsms-postbox-addon__status');
                    if (containStatus) {
                    ...
                    item.parentElement.classList.add('wpsms-hide');
                }
            });
        ...
        const processActiveSlugsSequentially = (index) => {
            ...
            processSlugs(activeSlugs, 'wp_statistics_activate_plugin', index, () => {
                active_addon_plugin_btn.forEach(function (item, i) {
                    item.parentElement.classList.remove('wpsms-hide')
                });
        ...
        if (addon_items.length > 0) {
            if (select_all) {
                select_all.addEventListener('click', function (event) {
                    ...
                    addon_items.forEach(function (item) {
                        item.checked = true;
                        item.parentElement.parentElement.parentElement.classList.add('wpsms-has-checkbox')
                    });
                    addon_download_btn.classList.remove('disabled');
                    updateSelectedSlugs();
                });
            ...
            if (select_all) select_all.classList.add('wpsms-hide');
        }
        ...
        document.addEventListener('click', function (event) {
            if (event.target.classList.contains('js-addon-retry-btn')) {
                ...
                event.stopPropagation();
                active_all.classList.add('wpsms-hide');
                        ...
                        if (closestItem) {
                            ...
                            if (active_input && active_button) {
                                function toggleButtonState() {
                                    active_input.classList.remove('wpsms-danger', 'wpsms-warning');
        ...
        const errorHandel = (params, button, data) => {
            ...
            if (params.action === "wp_statistics_download_plugin") {
                ...
                if (current_plugin_checkbox) {
                    ...
                    const statusSpan = document.createElement('span');
                    statusSpan.classList.add('wpsms-postbox-addon__status', 'wpsms-postbox-addon__status--danger');
                    ...
                    if (params.tab === 'get-started') {
                        ...
                        retryBtn.classList.add('wpsms-postbox-addon__button', 'button-retry-addon-download', 'js-addon-retry-btn');
        ...
        const sendAjaxRequest = (params, button, callback) => {
            ...
            if (params.action === "wp_statistics_download_plugin") {
                ...
                const current_plugin = document.querySelector(`[data-slug="${params.plugin_slug}"]`);
                if (current_plugin) {
                    const statusLable = current_plugin.parentElement.parentElement.querySelector('.wpsms-postbox-addon__status');
                    if (statusLable) {
                        statusLable.remove();
                    }
                    const statusSpan = document.createElement('span');
                    statusSpan.classList.add('wpsms-postbox-addon__status', 'wpsms-postbox-addon__status--purple');
            ...
            jQuery.ajax({
                ...
                success: function (data) {
                    if (button) button.classList.remove('wpsms-loading-button');
                    if (data.success) {
                        ...
                        if (params.action === "wp_statistics_check_license") {
                            if (wps_js.global.request_params?.tab) {
                                ...
                                toggleAlertBox(button);
                                button.parentElement.querySelector('input').classList.remove('wpsms-danger');
                                const statusDanger = button.parentElement.parentElement.parentElement.querySelector('.wpsms-postbox-addon__status');
                                if (statusDanger.classList.contains('wpsms-postbox-addon__status--danger')) {
                                    statusDanger.classList.add('wpsms-postbox-addon__status--success');
                                    statusDanger.classList.remove('wpsms-postbox-addon__status--danger');
                        ...
                        if (params.action === "wp_statistics_download_plugin") {
                            const current_plugin_checkbox = document.querySelector(`[data-slug="${params.plugin_slug}"]`);
                            if (current_plugin_checkbox) {
                                const loadingStatus = current_plugin_checkbox.parentElement.parentElement.querySelector('.wpsms-postbox-addon__status--purple');
                                const updatedLable = current_plugin_checkbox.parentElement.parentElement.querySelector('.wpsms-postbox-addon__label--updated');
                                if (loadingStatus) {
                                    loadingStatus.remove();
                                }
                                if (updatedLable) {
                                ...
                                const statusSpan = document.createElement('span');
                                statusSpan.classList.add('wpsms-postbox-addon__status', 'wpsms-postbox-addon__status--installed');
                                ...
                                if (params.tab === 'downloads') {
                                    statusSpan.parentElement.parentElement.classList.add('wpsms-addon__download__item--disabled');
                                }
                                if (params.tab === 'get-started') {
                                    const activeBtn = document.createElement('a');
                                    activeBtn.classList.add('wpsms-postbox-addon__button', 'button-activate-addon', 'js-addon-active-plugin-btn');
                        ...
                        if (params.action === "wp_statistics_activate_plugin") {
                            ...
                            if (current_plugin_checkbox) {
                                ...
                                const statusSpan = document.createElement('span');
                                statusSpan.classList.add('wpsms-postbox-addon__status', 'wpsms-postbox-addon__status--success');
