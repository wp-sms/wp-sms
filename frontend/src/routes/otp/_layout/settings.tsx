...
function RouteComponent() {
  const { data: result } = useSuspenseQuery(getSchemaByGroup({ groupName: 'otp-settings', include_hidden: true }))
  const { data: valuesResult } = useSuspenseQuery(getSettingsValuesByGroup({ groupName: 'otp-settings' }))
  const { mutateAsync } = useSaveSettingsValues({ groupName: 'otp-settings', include_hidden: true })
