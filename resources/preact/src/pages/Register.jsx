...
import { registrationFields, registrationFieldDefs, formSlug, formRedirectUrl, formName, registrationEnabled, configLoading } from '../signals/config';
...
export function Register() {
    ...
    // Render a system field from its definition.
    function renderSystemField(def, isFirst) {
        const req = def.required ? <span className="wsms-auth-required"> *</span> : null;
        switch (def.id) {
            case 'username':
                return (
                    <div className="wsms-auth-stack-2" key="username">
                        <Label for="wsms-reg-username">{__('Username', 'wp-sms')}{req}</Label>
                        <Input
            ...
            case 'display_name':
                return (
                    <div className="wsms-auth-stack-2" key="display_name">
                        <Label for="wsms-reg-name">{__('Display Name', 'wp-sms')}{req}</Label>
                        <Input
                            ref={isFirst ? firstFieldRef : undefined}
                            id="wsms-reg-name"
                            type="text"
            ...
            case 'first_name':
                return (
                    <div className="wsms-auth-stack-2" key="first_name">
                        <Label for="wsms-reg-first-name">{__('First Name', 'wp-sms')}{req}</Label>
                        <Input
                            ref={isFirst ? firstFieldRef : undefined}
            ...
            case 'last_name':
                return (
                    <div className="wsms-auth-stack-2" key="last_name">
                        <Label for="wsms-reg-last-name">{__('Last Name', 'wp-sms')}{req}</Label>
                        <Input
                            ref={isFirst ? firstFieldRef : undefined}
            ...
            case 'email':
                return (
                    <div className="wsms-auth-stack-2" key="email">
                        <Label for="wsms-reg-email">{__('Email', 'wp-sms')}{req}</Label>
                        <Input
                            ref={isFirst ? firstFieldRef : undefined}
                            id="wsms-reg-email"
                            type="email"
            ...
            case 'phone':
                return (
                    <div className="wsms-auth-stack-2" key="phone">
                        <Label for="wsms-reg-phone">{__('Phone Number', 'wp-sms')}{req}</Label>
                        <PhoneInput
                            id="wsms-reg-phone"
                            value={form.phone}
            ...
            case 'password':
                return (
                    <div className="wsms-auth-stack-2" key="password">
                        <Label for="wsms-reg-password">{__('Password', 'wp-sms')}{req}</Label>
                        <PasswordInput
            ...
            <form onSubmit={handleSubmit} className="wsms-auth-stack-4">
                ...
                {fieldDefs.map((def, idx, arr) => {
                    if (SYSTEM_FIELD_IDS.includes(def.id)) {
                        if (def.id === 'first_name' && arr[idx + 1]?.id === 'last_name') {
                            return (
                                <div className="wsms-auth-profile-grid" key="name-pair">
                                    {renderSystemField(def, idx === 0)}
                                    {renderSystemField(arr[idx + 1], false)}
                                </div>
                            );
                        }
                        ...
                        return renderSystemField(def, idx === 0);
                    }
                    return (
                        <DynamicField
                            key={def.id}
