...
class mtarget extends \WP_SMS\Gateway
{
    ...
    public function SendSMS()
    {
        ...
        foreach ($msisdns_sublists as $sublist) {
            ...
            $result = $this->request('GET', $this->wsdl_link . '?username=' . urlencode($this->username) . '&password=' . urlencode($this->password) . '&sender=' . urlencode($this->from) . '&msisdn=' . urlencode($to_list) . '&msg=' . urlencode($this->msg) . '&allowunicode=' . $allowunicode);
    ...
    public function GetCredit()
    {
        ...
        $result = $this->request('GET', "https://smswebservices.mtarget.fr/SmsWebServices/ServletSms?method=getAccountInformation&username=" . $this->username . "&password=" . $this->password);
