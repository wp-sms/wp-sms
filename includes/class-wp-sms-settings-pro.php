...
class WP_SMS_Settings_Pro {
	...
	public function add_settings_menu() {
		add_submenu_page('wp-sms', __('Pro Package', 'wp-sms'), '<span style="color:#FF7600">'.__('Pro Package', 'wp-sms').'</span>', 'wpsms_setting', 'wp-sms-pro', array(&$this, 'render_settings'));
	}
	...
	public function get_registered_settings() {
		...
		if(class_exists('RGFormsModel')) {
			...
			foreach( $forms as $form ):
				...
				if(WP_SMS_Pro_Gravityforms::get_field($form->id)) {
					...
					$gf_forms['gf_notify_receiver_field_form_' . $form->id] =  array(
						...
						'type'		=>  'select',
						'options'	=>  WP_SMS_Pro_Gravityforms::get_field($form->id),
			...
			foreach( $forms as $form ):
				...
				if($form['elements']) {
					...
					$qf_forms['qf_notify_receiver_field_form_' . $form['id']] =  array(
						...
						'name'		=>  __( 'Field form', 'wp-sms' ),
						'type'		=>  'select',
						'options'	=>  WP_SMS_Pro_Quform::get_field($form['id']),
						'desc'		=>  __( 'Please select the field of the form', 'wp-sms' )
					);
		...
		$settings = apply_filters( 'wps_pp_registered_settings', array(
			...
			'wc'                  =>  apply_filters( 'wps_pp_wc_settings', array(
				...
				'wc_notify_product_receiver'        =>  array(
					...
					'type'          =>  'select',
					'options'       =>  array('subscriber' => __('Subscribe users', 'wp-sms'), 'users' => __('Customers (Users)', 'wp-sms')),
	...
	public function rich_editor_callback( $args ) { $wp_version;
