...
class Front
{
    ...
    public function front_assets()
    {
        ...
        if (is_admin_bar_showing()) {
            wp_register_style('wpsms-admin-bar', WP_SMS_URL . 'public/legacy/css/admin-bar.css', true, WP_SMS_VERSION);
        ...
        if (!wp_sms_get_option('disable_style_in_front')) {
            wp_register_style('wpsms-front', WP_SMS_URL . 'public/legacy/css/front-styles.min.css', true, WP_SMS_VERSION);
            wp_enqueue_style('wpsms-front');
        }

        // Register subscriber form script
        wp_register_script('wp-sms-front-script', WP_SMS_URL . 'public/legacy/js/frontend.min.js', ['jquery'], WP_SMS_VERSION, true);
