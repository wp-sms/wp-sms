...
class DashboardHandler extends BaseAssets
{
    ...
    /**
     * Attach WordPress.org JSON translations to the dashboard.
     *
     * The generated i18n-strings.js contains every React label under one stable
     * script path. Registering that real path lets wp_set_script_translations()
     * load the matching language-pack JSON before the dashboard module runs.
     *
     * @return void
     */
    private function enqueueScriptTranslations()
    {
        $handle   = 'wsms-dashboard-i18n';
        $file     = 'i18n-strings.js';
        $filePath = $this->getUrl($file, true);
        ...
        wp_set_script_translations($handle, 'wp-sms');
    }
