...
class DashboardHandler extends BaseAssets
{
    ...
    private function enqueueProductionAssets($manifestPath, $distUrl)
    {
        ...
        if (function_exists('wp_enqueue_script_module')) {
            ...
            $this->enqueueScriptTranslations();
        } else {
            // Fallback for WP < 6.5
    ...
    /**
     * Attach JSON translations to the dashboard bundle.
     *
     * The React app consumes `wp.i18n` via a Vite alias shim. wp_set_script_translations()
     * can't be used directly here because (1) it only works with classic script handles,
     * not script modules, and (2) its file lookup is keyed by an md5 of the script src
     * path — which for a virtual anchor handle would never match the md5s recorded for
     * the actual React source files.
     *
     * Instead we merge every `wp-sms-{locale}-*.json` file generated by
     * `wp i18n make-json` and inject a single `wp.i18n.setLocaleData()` call inline on
     * the real `wp-i18n` handle, which loads before the dashboard module in the footer.
     *
     * @return void
     */
    private function enqueueScriptTranslations()
    {
        wp_enqueue_script('wp-i18n');

        $localeData = $this->loadDashboardLocaleData();
        if (empty($localeData)) {
            return;
        }

        $inline = sprintf(
            '( function( domain, localeData ) { localeData[""].domain = domain; wp.i18n.setLocaleData( localeData, domain ); } )( "wp-sms", %s );',
            wp_json_encode($localeData)
        );
        wp_add_inline_script('wp-i18n', $inline, 'after');
    }

    /**
     * Collect and merge every Jed-formatted JSON translation file in the plugin's
     * languages directory that targets the current locale and the wp-sms domain.
     *
     * @return array<string, mixed> Jed `locale_data.messages` object, or empty array
     *                              when no translations are installed.
     */
    private function loadDashboardLocaleData()
    {
        static $cache = [];
        ...
        $prefix = 'wp-sms-' . $locale . '-';
        $dirs   = [
            WP_SMS_DIR . 'resources/languages',
            WP_LANG_DIR . '/plugins',
        ];
        ...
        foreach ($dirs as $dir) {
            foreach (glob($dir . '/' . $prefix . '*.json') ?: [] as $file) {
                ...
                $messages = $data['locale_data']['messages'] ?? reset($data['locale_data']);
                ...
                if (isset($messages['']['plural-forms']) && !isset($merged['']['plural-forms'])) {
                    $merged['']['plural-forms'] = $messages['']['plural-forms'];
                }
            }
        }

        return $cache[$locale] = count($merged) > 1 ? $merged : [];
    }
