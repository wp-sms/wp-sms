...
/**
 * ParsaSMS — Iranian SMS panel exposing a SOAP web service.
 *
 * SOAP endpoint: http://parsasms.com/webservice/v2.asmx?WSDL
 *
 * Operations:
 *   SendSMS({ username, password, senderNumbers[], recipientNumbers[], messageBodies[] })
 ...
 */
class ParsaSmsProvider extends AbstractProvider
{
    /** Flip to true once the gateway clears end-to-end manual verification. */
    public const TESTED = false;

    private const WSDL = 'http://parsasms.com/webservice/v2.asmx?WSDL';
        ...
        return [
            'shared' => [
                'username' => [
                    ...
                    'required'    => true,
                    'description' => __('Your ParsaSMS panel username', 'wp-sms'),
                ],
                'password' => [
                    'type'        => 'secret',
                    'label'       => __('API Password', 'wp-sms'),
                    'required'    => true,
                    'description' => __('Your ParsaSMS panel password', 'wp-sms'),
                ],
            ],
            'channels' => [
                'sms' => [
                    'sender' => [
                        ...
                        'required'    => true,
                        'description' => __('Dedicated line purchased from the ParsaSMS panel', 'wp-sms'),
                    ],
                ],
            ],
        ];
    ...
    protected function doSend(MessageInterface $message): DeliveryResult
    {
        if (!class_exists('SoapClient')) {
            return DeliveryResult::failed(__('PHP SOAP extension is required for the ParsaSMS gateway', 'wp-sms'));
        }
        ...
        if ($username === '' || $password === '') {
            return DeliveryResult::failed(__('ParsaSMS credentials not configured', 'wp-sms'));
        }
        if ($sender === '') {
            return DeliveryResult::failed(__('ParsaSMS sender not configured', 'wp-sms'));
        }

        $params = [
            'username'         => $username,
            'password'         => $password,
            'senderNumbers'    => [$sender],
            'recipientNumbers' => [$message->getRecipient()],
            'messageBodies'    => [$message->getBody()],
        ];

        return $this->callSoap('SendSMS', $params, function ($response) {
            $result = $response->SendSMSResult ?? null;
            return $this->parseResponse($result, __('ParsaSMS send failed', 'wp-sms'));
        });
    }
    ...
    {
        if (!class_exists('SoapClient')) {
            return TestConnectionResult::error(__('PHP SOAP extension is required for the ParsaSMS gateway', 'wp-sms'));
        }
