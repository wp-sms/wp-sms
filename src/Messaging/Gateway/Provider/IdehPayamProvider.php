...
/**
 * Ideh Payam — Iranian SMS panel exposing a SOAP web service.
 *
 * SOAP endpoint: http://87.107.124.66/webservice/send.php?wsdl
 *
 * Operations:
 ...
 */
class IdehPayamProvider extends AbstractProvider
{
    /** Flip to true once the gateway clears end-to-end manual verification. */
    public const TESTED = false;

    private const WSDL = 'http://87.107.124.66/webservice/send.php?wsdl';
        ...
        return [
            'shared' => [
                'username' => [
                    ...
                    'required'    => true,
                    'description' => __('Your Ideh Payam panel username', 'wp-sms'),
                ],
                'password' => [
                    'type'        => 'secret',
                    'label'       => __('API Password', 'wp-sms'),
                    'required'    => true,
                    'description' => __('Your Ideh Payam panel password', 'wp-sms'),
                ],
            ],
            'channels' => [
                'sms' => [
                    'sender' => [
                        ...
                        'required'    => true,
                        'description' => __('Dedicated line purchased from the Ideh Payam panel', 'wp-sms'),
                    ],
                ],
            ],
        ];
    ...
    protected function doSend(MessageInterface $message): DeliveryResult
    {
        if (!class_exists('SoapClient')) {
            return DeliveryResult::failed(__('PHP SOAP extension is required for the Ideh Payam gateway', 'wp-sms'));
        }
        ...
        if ($username === '' || $password === '') {
            return DeliveryResult::failed(__('Ideh Payam credentials not configured', 'wp-sms'));
        }
        if ($sender === '') {
            return DeliveryResult::failed(__('Ideh Payam sender not configured', 'wp-sms'));
        }
    ...
    private function parseSendResponse(mixed $response): DeliveryResult
    {
        ...
        return DeliveryResult::failed((string) ($response ?? __('Ideh Payam send failed', 'wp-sms')));
    }
    ...
    {
        if (!class_exists('SoapClient')) {
            return TestConnectionResult::error(__('PHP SOAP extension is required for the Ideh Payam gateway', 'wp-sms'));
        }
