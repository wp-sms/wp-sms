...
/**
 * Farapayamak — Iranian SMS panel running on the Payamak Panel platform.
 *
 * REST API verified against the official Melipayamak Python SDK and the
 * third-party PayamakCore .NET SDK:
 ...
 */
class FarapayamakProvider extends AbstractProvider implements SupportsTemplates
{
        ...
        return [
            'shared' => [
                'username' => [
                    ...
                    'required'    => true,
                    'description' => __('Your Farapayamak panel username', 'wp-sms'),
                ],
                'password' => [
                    'type'        => 'secret',
                    'label'       => __('API Password', 'wp-sms'),
                    'required'    => true,
                    'description' => __('Your Farapayamak panel password', 'wp-sms'),
                ],
            ],
            'channels' => [
                'sms' => [
                    'sender' => [
                        ...
                        'required'    => true,
                        'description' => __('Dedicated line purchased from the Farapayamak panel', 'wp-sms'),
    ...
    protected function doSend(MessageInterface $message): DeliveryResult
    {
        ...
        if ($username === '' || $password === '') {
            return DeliveryResult::failed(__('Farapayamak credentials not configured', 'wp-sms'));
        }
        ...
        if ($sender === '') {
            return DeliveryResult::failed(__('Farapayamak sender not configured', 'wp-sms'));
        }
    ...
    private function parseResponse(array|DeliveryResult $result): DeliveryResult
    {
        ...
        if (!is_array($data)) {
            return DeliveryResult::failed(__('Invalid response from Farapayamak', 'wp-sms'));
        }
        ...
        return DeliveryResult::failed((string) ($data['StrRetStatus'] ?? __('Farapayamak send failed', 'wp-sms')));
    }
    ...
    public function testConnection(): TestConnectionResult
    {
        ...
        $data = $this->validateTestResponse($result, 'Farapayamak');
