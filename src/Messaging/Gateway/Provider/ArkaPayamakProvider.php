...
/**
 * ArkaPayamak — Iranian SMS reseller running on the Payamak Panel SOAP service.
 *
 ...
 *     -> SendSmsResult (numeric message id, or error string)
 *   GetCredit({ username, password })
 *     -> GetCreditResult (numeric credit)
 *
 * Auth: account username + password sent in every SOAP request body.
 *
 * Out of scope: templates/patterns (the Payamak Panel REST API exposes
 * BaseServiceNumber, but the SOAP endpoint used by this v7 driver does
 * not), MMS, status webhooks, inbound webhooks. Requires the PHP soap
 * extension.
 *
 ...
 */
class ArkaPayamakProvider extends AbstractProvider
{
        ...
        return [
            'shared' => [
                'username' => [
                    ...
                    'required'    => true,
                    'description' => __('Your ArkaPayamak panel username', 'wp-sms'),
                ],
                'password' => [
                    'type'        => 'secret',
                    'label'       => __('API Password', 'wp-sms'),
                    'required'    => true,
                    'description' => __('Your ArkaPayamak panel password', 'wp-sms'),
                ],
            ],
            'channels' => [
                'sms' => [
                    'sender' => [
                        ...
                        'required'    => true,
                        'description' => __('Dedicated line purchased from the ArkaPayamak panel', 'wp-sms'),
                    ],
                ],
            ],
        ];
    ...
    protected function doSend(MessageInterface $message): DeliveryResult
    {
        if (!class_exists('SoapClient')) {
            return DeliveryResult::failed(__('PHP SOAP extension is required for the ArkaPayamak gateway', 'wp-sms'));
        }
        ...
        if ($username === '' || $password === '') {
            return DeliveryResult::failed(__('ArkaPayamak credentials not configured', 'wp-sms'));
        }
        if ($sender === '') {
            return DeliveryResult::failed(__('ArkaPayamak sender not configured', 'wp-sms'));
        }
        ...
        return $this->callSoap('SendSms', $params, function ($response) {
            ...
            return $this->parseResponse($result, __('ArkaPayamak send failed', 'wp-sms'));
        });
    }
    ...
    {
        if (!class_exists('SoapClient')) {
            return TestConnectionResult::error(__('PHP SOAP extension is required for the ArkaPayamak gateway', 'wp-sms'));
        }
