...
/**
 * OnlinePanel — Iranian SMS panel exposing a SOAP web service.
 *
 * SOAP endpoint: http://87.107.121.52/post/send.asmx?WSDL
 *
 * Operations:
 ...
 *
 * Auth: account username + password sent in every SOAP request body. The
 * WSDL host is a bare IP address (no TLS / DNS name), preserved verbatim
 * from v7.
 *
 ...
 */
class OnlinePanelProvider extends AbstractProvider
{
    /** Flip to true once the gateway clears end-to-end manual verification. */
    public const TESTED = false;

    private const WSDL = 'http://87.107.121.52/post/send.asmx?WSDL';
        ...
        return [
            'shared' => [
                'username' => [
                    ...
                    'required'    => true,
                    'description' => __('Your OnlinePanel account username', 'wp-sms'),
                ],
                'password' => [
                    'type'        => 'secret',
                    'label'       => __('API Password', 'wp-sms'),
                    'required'    => true,
                    'description' => __('Your OnlinePanel account password', 'wp-sms'),
                ],
            ],
            'channels' => [
                'sms' => [
                    'sender' => [
                        ...
                        'required'    => true,
                        'description' => __('Dedicated line purchased from the OnlinePanel account', 'wp-sms'),
                    ],
                ],
            ],
        ];
    ...
    protected function doSend(MessageInterface $message): DeliveryResult
    {
        if (!class_exists('SoapClient')) {
            return DeliveryResult::failed(__('PHP SOAP extension is required for the OnlinePanel gateway', 'wp-sms'));
        }
        ...
        if ($username === '' || $password === '') {
            return DeliveryResult::failed(__('OnlinePanel credentials not configured', 'wp-sms'));
        }
        if ($sender === '') {
            return DeliveryResult::failed(__('OnlinePanel sender not configured', 'wp-sms'));
        }
        ...
        return $this->callSoap('SendSms', $params, function ($response) {
            ...
            return $this->parseResponse($result, __('OnlinePanel send failed', 'wp-sms'));
        });
    }
    ...
    {
        if (!class_exists('SoapClient')) {
            return TestConnectionResult::error(__('PHP SOAP extension is required for the OnlinePanel gateway', 'wp-sms'));
        }
