...
/**
 * Chapargah — Iranian SMS panel exposing a SOAP web service.
 *
 * SOAP endpoint: http://panel.chapargah.ir/API/Send.asmx?WSDL
 *
 * Operations:
 *   SendSms({ username, password, from, to[], text, flash, udh })
 *     -> SendSmsResult (string/numeric message id, or error string)
 *   Credit({ username, password })
 *     -> CreditResult (numeric credit)
 *
 ...
 */
class ChapargahProvider extends AbstractProvider
{
    /** Flip to true once the gateway clears end-to-end manual verification. */
    public const TESTED = false;

    private const WSDL = 'http://panel.chapargah.ir/API/Send.asmx?WSDL';
        ...
        return [
            'shared' => [
                'username' => [
                    ...
                    'required'    => true,
                    'description' => __('Your Chapargah panel username', 'wp-sms'),
                ],
                'password' => [
                    'type'        => 'secret',
                    'label'       => __('API Password', 'wp-sms'),
                    'required'    => true,
                    'description' => __('Your Chapargah panel password', 'wp-sms'),
                ],
            ],
            'channels' => [
                'sms' => [
                    'sender' => [
                        ...
                        'required'    => true,
                        'description' => __('Dedicated line purchased from the Chapargah panel', 'wp-sms'),
                    ],
                ],
            ],
        ];
    ...
    protected function doSend(MessageInterface $message): DeliveryResult
    {
        if (!class_exists('SoapClient')) {
            return DeliveryResult::failed(__('PHP SOAP extension is required for the Chapargah gateway', 'wp-sms'));
        }
        ...
        if ($username === '' || $password === '') {
            return DeliveryResult::failed(__('Chapargah credentials not configured', 'wp-sms'));
        }
        if ($sender === '') {
            return DeliveryResult::failed(__('Chapargah sender not configured', 'wp-sms'));
        }
        ...
        return $this->callSoap('SendSms', $params, function ($response) {
            ...
            return $this->parseResponse($result, __('Chapargah send failed', 'wp-sms'));
        });
    }
    ...
    private function parseResponse(mixed $result, string $defaultError): DeliveryResult
    {
        ...
        // Heuristic: numeric responses are message ids, non-numeric typically error strings.
        if (is_numeric($value)) {
    ...
    public function getCredit(): ?string
    {
        ...
        try {
            ...
            $response = $client->Credit(['username' => $username, 'password' => $password]);
        } catch (\Throwable $ex) {
            return null;
        }

        $credit = $response->CreditResult ?? null;
    ...
    {
        if (!class_exists('SoapClient')) {
            return TestConnectionResult::error(__('PHP SOAP extension is required for the Chapargah gateway', 'wp-sms'));
        }
