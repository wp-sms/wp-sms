...
/**
 * Paaz — Iranian SMS panel exposing a SOAP web service.
 *
 * SOAP endpoint: http://sms.paaz.ir/post/send.asmx?wsdl
 *
 * Operations:
 ...
 */
class PaazProvider extends AbstractProvider
{
    /** Flip to true once the gateway clears end-to-end manual verification. */
    public const TESTED = false;

    private const WSDL = 'http://sms.paaz.ir/post/send.asmx?wsdl';
        ...
        return [
            'shared' => [
                'username' => [
                    ...
                    'required'    => true,
                    'description' => __('Your Paaz panel username', 'wp-sms'),
                ],
                'password' => [
                    'type'        => 'secret',
                    'label'       => __('API Password', 'wp-sms'),
                    'required'    => true,
                    'description' => __('Your Paaz panel password', 'wp-sms'),
                ],
            ],
            'channels' => [
                'sms' => [
                    'sender' => [
                        ...
                        'required'    => true,
                        'description' => __('Dedicated line purchased from the Paaz panel', 'wp-sms'),
                    ],
                ],
            ],
        ];
    ...
    protected function doSend(MessageInterface $message): DeliveryResult
    {
        if (!class_exists('SoapClient')) {
            return DeliveryResult::failed(__('PHP SOAP extension is required for the Paaz gateway', 'wp-sms'));
        }
        ...
        if ($username === '' || $password === '') {
            return DeliveryResult::failed(__('Paaz credentials not configured', 'wp-sms'));
        }
        if ($sender === '') {
            return DeliveryResult::failed(__('Paaz sender not configured', 'wp-sms'));
        }
        ...
        return $this->callSoap('SendSms', $params, function ($response) {
            ...
            return $this->parseResponse($result, __('Paaz send failed', 'wp-sms'));
        });
    }
    ...
    {
        if (!class_exists('SoapClient')) {
            return TestConnectionResult::error(__('PHP SOAP extension is required for the Paaz gateway', 'wp-sms'));
        }
