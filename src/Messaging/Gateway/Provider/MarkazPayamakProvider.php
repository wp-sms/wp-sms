...
/**
 * Markaz Payamak — Iranian SMS reseller running on the legacy Payamak Panel
 * SOAP backend (same WSDL as PayamakPanelProvider; separate provider for
 * brand/credentials separation).
 *
 ...
 */
class MarkazPayamakProvider extends AbstractProvider
{
        ...
        return [
            'shared' => [
                'username' => [
                    ...
                    'required'    => true,
                    'description' => __('Your Markaz Payamak account username', 'wp-sms'),
                ],
                'password' => [
                    'type'        => 'secret',
                    'label'       => __('API Password', 'wp-sms'),
                    'required'    => true,
                    'description' => __('Your Markaz Payamak account password', 'wp-sms'),
                ],
            ],
            'channels' => [
                'sms' => [
                    'sender' => [
                        ...
                        'required'    => true,
                        'description' => __('Dedicated line purchased from the Markaz Payamak account', 'wp-sms'),
                    ],
                ],
            ],
        ];
    ...
    protected function doSend(MessageInterface $message): DeliveryResult
    {
        if (!class_exists('SoapClient')) {
            return DeliveryResult::failed(__('PHP SOAP extension is required for the Markaz Payamak gateway', 'wp-sms'));
        }
        ...
        if ($username === '' || $password === '') {
            return DeliveryResult::failed(__('Markaz Payamak credentials not configured', 'wp-sms'));
        }
        if ($sender === '') {
            return DeliveryResult::failed(__('Markaz Payamak sender not configured', 'wp-sms'));
        }
        ...
        return $this->callSoap('SendSms', $params, function ($response) {
            ...
            return $this->parseResponse($result, __('Markaz Payamak send failed', 'wp-sms'));
        });
    }
    ...
    {
        if (!class_exists('SoapClient')) {
            return TestConnectionResult::error(__('PHP SOAP extension is required for the Markaz Payamak gateway', 'wp-sms'));
        }
