...
/**
 * SMS Hooshmand — Iranian SMS panel exposing a SOAP/WSDL API.
 *
 * SOAP API:
 *   WSDL:    http://smshooshmand.com/class/sms/webservice3/server.php?wsdl
 ...
 * every call. The legacy v7 implementation used the bundled `nusoap_client`;
 * v8 uses PHP's built-in SoapClient (php-soap extension) instead.
 *
 * Response shape: SendSMS returns a vendor-specific identifier or numeric
 * status string; non-empty truthy value indicates success. GetCredit returns
 * a bare credit count.
 *
 * Out of scope (not exposed by the API): MMS, flash SMS toggle (the v7
 * `flash` flag is unused at the call level), templates, status webhooks,
 * inbound webhooks, opt-out detection.
 *
 ...
 */
class SmsHooshmandProvider extends AbstractProvider
{
    /** Flip to true once the gateway clears end-to-end manual verification. */
    public const TESTED = false;

    private const WSDL_URL = 'http://smshooshmand.com/class/sms/webservice3/server.php?wsdl';
        ...
        return [
            'shared' => [
                'username' => [
                    ...
                    'required'    => true,
                    'description' => __('Your SMS Hooshmand panel username', 'wp-sms'),
                ],
                'password' => [
                    'type'        => 'secret',
                    'label'       => __('API Password', 'wp-sms'),
                    'required'    => true,
                    'description' => __('Your SMS Hooshmand panel password', 'wp-sms'),
                ],
            ],
            'channels' => [
                'sms' => [
                    'sender' => [
                        ...
                        'required'    => true,
                        'description' => __('Dedicated line purchased from the SMS Hooshmand panel', 'wp-sms'),
                    ],
                ],
            ],
        ];
    ...
    protected function doSend(MessageInterface $message): DeliveryResult
    {
        ...
        if ($username === '' || $password === '') {
            return DeliveryResult::failed(__('SMS Hooshmand credentials not configured', 'wp-sms'));
        }

        $sender = (string) $this->getChannelConfig('sms', 'sender', '');
        if ($sender === '') {
            return DeliveryResult::failed(__('SMS Hooshmand sender not configured', 'wp-sms'));
        }
    ...
    private function parseResponse(mixed $result): DeliveryResult
    {
        ...
        if ($result === null || $result === '' || $result === false) {
            return DeliveryResult::failed(__('SMS Hooshmand send failed', 'wp-sms'));
        }
