...
/**
 * Razpayamak — Iranian SMS panel running on the Payamak Panel platform.
 *
 ...
 */
class RazpayamakProvider extends AbstractProvider implements SupportsTemplates
{
        ...
        return [
            'shared' => [
                'username' => [
                    ...
                    'required'    => true,
                    'description' => __('Your Razpayamak panel username', 'wp-sms'),
                ],
                'password' => [
                    'type'        => 'secret',
                    'label'       => __('API Password', 'wp-sms'),
                    'required'    => true,
                    'description' => __('Your Razpayamak panel password', 'wp-sms'),
                ],
            ],
            'channels' => [
                'sms' => [
                    'sender' => [
                        ...
                        'required'    => true,
                        'description' => __('Dedicated line purchased from the Razpayamak panel', 'wp-sms'),
    ...
    protected function doSend(MessageInterface $message): DeliveryResult
    {
        ...
        if ($username === '' || $password === '') {
            return DeliveryResult::failed(__('Razpayamak credentials not configured', 'wp-sms'));
        }
        ...
        if ($sender === '') {
            return DeliveryResult::failed(__('Razpayamak sender not configured', 'wp-sms'));
        }
    ...
    private function parseResponse(array|DeliveryResult $result): DeliveryResult
    {
        ...
        if (!is_array($data)) {
            return DeliveryResult::failed(__('Invalid response from Razpayamak', 'wp-sms'));
        }
        ...
        return DeliveryResult::failed((string) ($data['StrRetStatus'] ?? __('Razpayamak send failed', 'wp-sms')));
    }
    ...
    public function testConnection(): TestConnectionResult
    {
        ...
        $data = $this->validateTestResponse($result, 'Razpayamak');
