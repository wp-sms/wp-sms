...
/**
 * SSMSS — Iranian SMS panel exposing a SOAP/WSDL API.
 *
 * SOAP API:
 *   WSDL:    http://www.ssmss.ir/webservice/smsService.php?wsdl
 ...
 */
class SsmssProvider extends AbstractProvider
{
    /** Flip to true once the gateway clears end-to-end manual verification. */
    public const TESTED = false;

    private const WSDL_URL = 'http://www.ssmss.ir/webservice/smsService.php?wsdl';
        ...
        return [
            'shared' => [
                'username' => [
                    ...
                    'required'    => true,
                    'description' => __('Your SSMSS panel username', 'wp-sms'),
                ],
                'password' => [
                    'type'        => 'secret',
                    'label'       => __('API Password', 'wp-sms'),
                    'required'    => true,
                    'description' => __('Your SSMSS panel password', 'wp-sms'),
                ],
            ],
            'channels' => [
                'sms' => [
                    'sender' => [
                        ...
                        'required'    => true,
                        'description' => __('Dedicated line purchased from the SSMSS panel', 'wp-sms'),
                    ],
                ],
            ],
        ];
    ...
    protected function doSend(MessageInterface $message): DeliveryResult
    {
        ...
        if ($username === '' || $password === '') {
            return DeliveryResult::failed(__('SSMSS credentials not configured', 'wp-sms'));
        }

        $sender = (string) $this->getChannelConfig('sms', 'sender', '');
        if ($sender === '') {
            return DeliveryResult::failed(__('SSMSS sender not configured', 'wp-sms'));
        }
        ...
        if ($result === null || $result === '' || $result === false) {
            return DeliveryResult::failed(__('SSMSS send failed', 'wp-sms'));
        }
