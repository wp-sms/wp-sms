...
class SchemaMaintainer
{
     ...
     * @var string
     */
    private const SCHEMA_CHECK_SUCCESS = 'wp_sms_schema_check_success';

    /**
     * Inspects database tables and returns any structural issues found.
     *
     * @param bool $forceRecheck Force a fresh check, ignoring cached results.
     * @return array Schema inspection results with status, issues, and errors.
     */
            ...
            foreach ($tableNames as $tableName) {
                ...
                try {
                    ...
                    if (strpos($e->getMessage(), 'does not exist') !== false) {
                        ...
                        error_log('[WP SMS] Schema check error: ' . $e->getMessage());
                        continue;
                    }
                    error_log('[WP SMS] Schema check error: ' . $e->getMessage());
                }
        ...
        try {
            ...
            foreach ($checkResults['issues'] as $issue) {
                try {
                    ...
                    error_log('[WP SMS] Schema repair error: ' . $e->getMessage());
                }
            }
