...
namespace WP_SMS\Services\OTP\Admin\Pages;

use WP_SMS\Contracts\Abstracts\AbstractService;

class OTPAdminPage extends AbstractService
{
    protected string $pageTitle = 'OTP/MFA Settings';
    protected string $menuTitle = 'OTP/MFA';
    protected string $capability = 'manage_options';
    protected string $menuSlug = 'wp-sms-otp-settings';

    public function getSlug(): string
    {
        return 'otp-admin-page';
    }
    ...
    public function registerMenu(): void
    {
        add_menu_page(
            ...
            $this->menuSlug,
            [$this, 'render'],
            'dashicons-shield', // WordPress security icon
            80 // Menu position
        );
    }

    /**
     * Remove admin notices from other plugins on WP-SMS OTP page
     */
    ...
    public function enqueueAssets(): void
    {
        ...
        $entry_key = 'src/pages/otp/index.tsx';
        ...
        if (!empty($entry['imports']) && is_array($entry['imports'])) {
            foreach ($entry['imports'] as $import_key) {
                if (!empty($manifest[$import_key]['file'])) {
                    $handle = 'wp-sms-otp-import-' . md5($import_key);
    ...
    public function render(): void
    {
        ...
        echo '<div class="wrap wp-sms-otp-wrap">';
        echo '<div id="wp-sms-otp-app"></div>';
        echo '</div>';
    }
}
