...
- Auth pages share `public/auth/` — **never run a single auth build in isolation**, always build all four together:
  ```bash
  npx vite build --config vite.config.auth.mjs && \
  npx vite build --config vite.config.auth-popup.mjs && \
