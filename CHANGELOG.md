v7.2.2 - 2026-03-**
- **Enhancement:** Redesigned the phone number migration wizard with a 5-step flow (Intro → Country → Review → Preview → Apply → Done), step indicator, in-flight progress bar, and explicit safety messaging shown BEFORE the user commits. Revert is now wired through a confirmation dialog instead of a one-click footer button, the dashboard notice is dynamic ("Improve delivery reliability for N phone numbers"), and the modal can no longer be dismissed mid-execute. Backend additions: scan returns `samples` / `total_records` / `previous_run_sources` / `cc_changed_since_last_run`, execute returns formatted backup timestamps, status reports a `running` flag for concurrency-collision recovery, and a new `clear_backup` sub-action lets admins drop a stale backup. Lock TTL is now refreshed per batch so very-large migrations can't lose their lock mid-flight.
